...
from gev.pool import Pool
from gev.server import StreamServer
from gev.socket import wait_write, socket
from gev import pywsgi
...
VERSION = "gevent/%s gunicorn/%s" % (gev.__version__, gunicorn.__version__)
...
class GeventWorker(AsyncWorker):
    ...
    def patch(self):
        from gev import monkey
        monkey.noisy = False

        # if the new version is used make sure to patch subprocess
        if gev.version_info[0] == 0:
            monkey.patch_all()
        else:
    ...
    def timeout_ctx(self):
        return gev.Timeout(self.cfg.keepalive, False)
    ...
    if gev.version_info[0] == 0:
        ...
        def init_process(self):
            ...
            gev.core.dns_shutdown(fail_requests=1)
            gev.core.dns_init()
